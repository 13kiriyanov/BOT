...
6. Реакция на TRENDING по умолчанию выключена: на 2000 окон эффект
   статистически неразличим от нуля (см. README). Детектор по собственным
   филлам структурно опаздывает — к входу позиция уже у лимита net, урон
   тренда ограничивает риск-лимит, а не котировки. Быстрее его сделал бы
   только рыночный поток целиком (публичные трейды) — его в боте нет.
7. Симулятор не моделирует разгрузку перекоса (build_unwind_quotes движка):
   в трендовых окнах он едет в лимит net до резолюции, то есть завышает
   урон тренда относительно реального бота и занижает ценность реакции.
